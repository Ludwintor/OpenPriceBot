    ...
    public static class Program
    {
        ...
        static Program()
        {
            _urls = new(new[]
            {
                ...
                new[]
                {
                    InlineKeyboardButton.WithUrl("Buy on Dedust", "https://dedust.io/swap/TON/EQDf84FT8tdHZeI2-LXdb8gPMRqHRSABrmi8jI7MzvVpGJKZ")
                }
            });
        }
        ...
        private static void Main(string[] args)
        {
            ...
            if (string.IsNullOrEmpty(token))
                throw new ArgumentNullException(nameof(token), $"Set telegram bot token to environment variable \"{BOT_TOKEN_ENV}\" or provide it as first argument");
        ...
        private static async Task Run()
        {
            ...
            while (true)
            {
                sb.Clear();
                Trade[]? trades = null;
                Pool? tonUsdtPool = null;
                Pool? openTonPool = null;
                try
                {
                ...
                if (trades != null && trades.Length > 0)
                {
                    ...
                    if (openTonPool != null)
                    {
                        double tonPrice = openTonPool.Value.CalculateRightToLeft(1d, null, 5);
                        double usdtPrice = tonUsdtPool?.CalculateLeftToRight(tonPrice) ?? 0d;
                        double priceChange = _lastTonPrice != 0d ? tonPrice / _lastTonPrice - 1d : 0d;
